...
local function dialog(o, text)
	...
	for po in utf8.codes(text) do
		...
		if #text ~= po then
			...
			dialog_text = '<span foreground="black">' .. vis .. '<span alpha="1">' .. invis .. '</span></span>'
		else
			dialog_text = '<span foreground="black">' .. text .. '</span>'
		end
	...
	final = Image.arrayjoin(frames, {across = 1}):flatten{background = {54, 57, 63}}
